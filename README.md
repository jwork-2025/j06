...
1) 参考本仓库代码，完善你自己的游戏：
   - 使用 Scene/GameObject/Component/System 的分层思路重构核心流程
   - 合理并行：用线程池处理可独立的批量计算（如物理/AI），注意线程安全

2) 为你的游戏设计并实现“存档与回放”功能：
   - 存档：定义存储抽象（文件/网络/内存均可），录制关键帧 + 输入/事件
   - 回放：读取存档，恢复对象状态并插值渲染，保证外观与行为可见且稳定

提示：请尽量保持模块解耦（渲染/输入/逻辑/存储），便于测试与替换实现。

\n
**重要提醒：本次作业请尽量手写代码，不依赖自动生成/粘贴现成实现，考试会考！**